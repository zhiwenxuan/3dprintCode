define([
    ...
    var EditingDOMView = VertexMV.EditingDOMView.extend({
      ...
      render: function() {
        var template =
          ...
          '</div>' +
          '<div>平台网格:<input class="field gridsize" type="text"/></div>' +
          '<div>原点 </div>' +
          '<div>' +
          ...
          '</div>' +
          '<div>角度 <input class="field angle" type="text" value="{{angle}}"></input></div>';
    ...
    var GridView = VertexMV.SceneView.extend({
      ...
      render: function() {
        ...
        var x,y,line;
        for (x = -Math.floor(boundary/grid); x <= Math.ceil(boundary/grid); ++x) {
          if ((x % 10 === 0) && (x !== 0)) {
            line = new THREE.Line(majorGridLineGeometry, majorMaterialInside);
        ...
        for (y = -Math.floor(boundary/grid); y <= Math.ceil(boundary/grid); ++y) {
          if ((y % 10 === 0) && (y !== 0)) {
            line = new THREE.Line(majorGridLineGeometry, majorMaterialInside);
        ...
        if (geometryGraph.isEditing()) {
          ...
          for (x = -Math.floor(boundary/grid); x <= Math.ceil(boundary/grid); ++x) {
            if (x % 10 !== 0) {
              line = new THREE.Line(minorGridLineGeometry, minorMaterialInside);
              line.position.x = x*grid;
              line.rotation.z = 90 * Math.PI / 180;
              this.sceneObject.add(line);
            }
      ...
      addAxes: function() {
        ...
        var headLength = 1;
    	var headWidth = 1;
		var arrowHelperX = new THREE.ArrowHelper( dirX, origin, length, 0x0000ff, headLength, headWidth ); 
		var arrowHelperY = new THREE.ArrowHelper( dirY, origin, length, 0x00ff00, headLength, headWidth ); 
		var arrowHelperZ = new THREE.ArrowHelper( dirZ, origin, length, hex, headLength, headWidth ); 
    	this.sceneObject.add( arrowHelperX );
    	this.sceneObject.add( arrowHelperY );
    	this.sceneObject.add( arrowHelperZ );

		axes[0].vertices.push(new THREE.Vector3(0, 0, 0));
        axes[0].vertices.push(new THREE.Vector3(grid*10*10, 0, 0));

        axes[1].vertices.push(new THREE.Vector3(0, 0, 0));
        axes[1].vertices.push(new THREE.Vector3(0, grid*10*10, 0));
        ...
        axes[3].vertices.push(new THREE.Vector3(-grid*10*10, 0, 0));

        axes[4].vertices.push(new THREE.Vector3(0, 0, 0));
        axes[4].vertices.push(new THREE.Vector3(0, -grid*10*10, 0));

        axes[5].vertices.push(new THREE.Vector3(0, 0, 0));
      //  axes[5].vertices.push(new THREE.Vector3(0, 0, 0));  // -Z axis
       //	axes[5].vertices.push(new THREE.ArrowHelper(dir,origin,length,hex) );  // -Z axis
        ...
        this.sceneObject.add(new THREE.Line(axes[1],
            new THREE.LineBasicMaterial({ linewidth:2, color: 0x00ff00 })));
        ...
        this.sceneObject.add(new THREE.Line(axes[3],
            new THREE.LineBasicMaterial({ linewidth:2, color: 0x6666cc })));
        this.sceneObject.add(new THREE.Line(axes[4],
            new THREE.LineBasicMaterial({ linewidth:2, color: 0x66cc66 })));
        this.sceneObject.add(new THREE.Line(axes[5],
            new THREE.LineBasicMaterial({ linewidth:2, color: 0xcc6666 })));
