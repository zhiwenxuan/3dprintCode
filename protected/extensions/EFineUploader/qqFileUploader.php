...
class qqFileUploader {
	...
	{   //get maximum file size upload limit from php.ini
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if ($name === null || $name === ''){
            return array('error' => '文件名不能为空');
        }

        // Validate file size

        if ($size == 0){
            return array('error' => '文件大小不能为空');
        }

        if ($size > $this->sizeLimit){
            return array('error' => '文件超过允许大小，请重新选择');
        }
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))){
            ...
            return array('error' => '上传文件格式不正确，目前仅支持'. $these . '格式');
        }
